module Shaders (initShaders) where
...
initShaders :: IO ()
initShaders = do
  program <- loadShaders [ ShaderInfo VertexShader (FileSource "shaders/simple_vertex_shader.glsl")
                         , ShaderInfo FragmentShader (FileSource "shaders/simple_fragment_shader.glsl")
                         ]
